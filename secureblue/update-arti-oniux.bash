...
# Automated script to install or update Rust-based Tor
# software (arti and oniux)
...
detect_root_cmd() {
	if [ "${EUID:-$(id -u)}" -eq 0 ]; then
		ROOT_CMD=""
		log "Running as root; no elevation helper" \
...
ensure_rust() {
	...
	if command -v brew-proxy >/dev/null 2>&1 ||
		...
		if "$brew_cmd" list rust >/dev/null 2>&1; then
			...
			"$brew_cmd" upgrade rust || log \
				"$brew_cmd upgrade rust failed or" \
				"was not needed."
		else
	...
	error "Rust installation seems to have failed" \
...
install_or_update_arti() {
	...
	if [[ -z $latest_tag ]]; then
		error "Could not determine latest arti tag;" \
			"installing from repo without explicit" \
			"tag."
		...
		if [[ $installed == "$latest_ver" ]]; then
			...
			log "Installing/updating arti to" \
				"version $latest_ver" \
				"(tag $latest_tag)" \
				"with --features=full..."
			cargo_np install --locked --features=full \
				--git "$REPO_ARTI" --tag "$latest_tag" \
				"$crate"
			updated=1
		fi
	fi
	...
	if [[ $updated -eq 1 ]]; then
		if [[ -x "$CARGO_BIN_DIR/arti" ]]; then
			...
			error "arti binary not found at" \
				"$CARGO_BIN_DIR/arti. Check" \
...
install_or_update_oniux() {
	...
	if [[ -z $latest_tag ]]; then
		error "Could not determine latest oniux" \
			"tag; installing from repo without" \
			"explicit tag."
		...
		if [[ $installed == "$latest_ver" ]]; then
			...
			log "Installing/updating oniux to" \
				"version $latest_ver" \
				"(tag $latest_tag)..."
			cargo_np install --locked \
				--git "$REPO_ONIUX" --tag "$latest_tag" \
				"$crate"
			updated=1
		fi
	fi
	...
	if [[ $updated -eq 1 ]]; then
		if [[ -x "$CARGO_BIN_DIR/oniux" ]]; then
			...
			error "oniux binary not found at" \
				"$CARGO_BIN_DIR/oniux. Check" \
				"previous error messages."
			return 1
		fi
	else
