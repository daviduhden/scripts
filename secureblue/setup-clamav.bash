...
ensure_kv_line() {
	...
	if grep -Eq "^[[:space:]]*#?[[:space:]]*${key}\b" "$file"; then sed -ri "s|^[[:space:]]*#?[[:space:]]*${key}\b.*|${key} ${value}|g" "$file"; else printf "\n%s %s\n" "$key" "$value" >>"$file"; fi
}
ensure_multi_line() {
	local file="$1" key="$2" value="$3"
	grep -Fxq "${key} ${value}" "$file" || printf "%s %s\n" "$key" "$value" >>"$file"
}
comment_out_example() {
	local file="$1"
...
start_clamd() {
	...
	until ((tries >= 3)); do
		...
		if systemctl restart clamd@scan.service && systemctl is-active --quiet clamd@scan.service; then
			log "clamd active"
			return 0
		else
			warn "clamd failed to start, retrying..."
		fi
		sleep 5
	done
...
configure_clamonacc() {
	...
	[[ -z $unit ]] && {
		log "No clamonacc service, skipping"
		return
	}
