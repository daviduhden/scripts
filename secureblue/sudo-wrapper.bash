...
# and wraps visudo via run0, and sudoedit via run0edit
# (safe graphical/root editor).
#
...
# - Scripts that call /usr/bin/sudo explicitly will still
...
handle_visudo() {
	...
	if [[ ! -x $real_visudo ]]; then
		error "sudo-wrapper error: could not locate the" \
			...
			"/usr/sbin/visudo or another executable" \
			"visudo in PATH."
	fi
