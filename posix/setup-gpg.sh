...
#       Forces branch and disables the interactive question on Ubuntu/Devuan.
...
GnuPG official repo options (Debian/Ubuntu/Devuan):
  --gnupg-branch stable|devel
      ...
      - On Ubuntu/Devuan, this disables the interactive branch question.
...
install_gnupg_debian_like() {
    ...
    if [ -n "$gnupg_branch_cli" ]; then
        ...
        if [ "$ID" = "debian" ]; then
            ...
            # Ubuntu/Devuan: interactive question if no CLI override
